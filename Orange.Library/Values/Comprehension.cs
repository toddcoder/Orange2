...
namespace Orange.Library.Values
{
	public class Comprehension : Value
	{
	   ...
	   public Value If()
		...
		Array getArray()
		{
			...
			if (value == null || value.Type == ValueType.Nil)
				return new Array();
		   var sequenceSource = value.As<ISequenceSource>();
			if (sequenceSource.IsSome)
			{
				var pipeline = new Sequence(sequenceSource.Value);
				var sourceArray = (Array)pipeline.SourceArray;
				return sourceArray;
			}
			var array = value.IsArray ? (Array)value.SourceArray : new Array
			{
				value
			};
			return array;
	   ...
	   public void PushDownInnerComprehension(Comprehension comprehension)
		...
		public Value From()
		{
			var comprehension = new Comprehension(block, Arguments.Parameters)
			{
		...
		{
			var comprehension = new Comprehension(block, lambda.Parameters)
			{
				ArrayBlock = lambda.Block
			};
			PushDownInnerComprehension(comprehension);
			block = null;
			return this;
