...
namespace Orange.Library.Values
{
   public class Parameter
   {
      ...
      public Parameter Clone() => new Parameter(Name.Copy(), (Block)DefaultValue?.Clone(), Visibility, ReadOnly, Lazy,
         Comparisand, (Block)Condition?.Clone());

      public void Assign(Value value, bool readOnly, bool setting, bool _override = false)
      {
         if (setting)
            Regions[Name] = ConvertIfNumeric(value);
         else
            Regions.SetParameter(Name, ConvertIfNumeric(value), _override: _override);
         if (readOnly)
   ...
   public class ParameterValue
   {
      ...
      public ParameterValue(string name, Block block, VisibilityType visibility, bool lazy)
      {
         ...
         string boundName;
         Value innerValue;
         Bound = Value.Unbind().Assign(out boundName, out innerValue);
         if (Bound)
         {
            BoundName = boundName;
            Value = innerValue;
   ...
   public class Parameters : Value
   {
      ...
      {
         parameters = array.Select(i => new Parameter(i.Value.Text)).ToArray();
      }
      ...
      public Parameters(string[] fieldNames)
      {
         parameters = fieldNames.Select(fn => new Parameter(fn)).ToArray();
      }
      ...
      public Parameter this[int index]
      {
         get
         {
            return parameters.Of(index, null);
         }
         set
         {
      ...
      public Value Assign()
      {
         ...
         {
            var value = MessageManager.MessagingState.Send(self, variableName, new Arguments());
            Variable variable;
      ...
      public override string ToString() => Pattern?.ToString() ?? parameters.Listify();

      public string VariableName(int index, string defaultValue = "")
      {
         return parameters.Of(index).Map(p => p.Name, () => defaultValue);
      }
      ...
      public override Value SourceArray
      {
         get
         {
            ...
            foreach (var parameter in parameters)
            {
               ...
               var nameGraph = new ValueGraph(parameter.Name)
               {
                  Value = parameter.Name
               };
               var defaultGraph = new ValueGraph("default")
               {
                  Value = parameter.DefaultValue
               };
               var isLocalGraph = new ValueGraph("visibility")
               {
                  Value = parameter.Visibility.ToString()
               };
      ...
      public void SetValues(Value value, int index = -1)
      {
         ...
         if (value is OTuple || value is Array)
         {
         ...
         var keyValue = value.As<KeyedValue>();
         if (keyValue.IsSome)
         {
            var key = keyValue.Value.Key;
            ...
            switch (names.Length)
            {
               case 0:
                  break;
               case 1:
                  current.SetParameter(names[0], value, _override: true);
                  break;
               case 2:
                  current.SetParameter(names[0], value, _override: true);
                  current.SetParameter(names[1], key, _override: true);
                  break;
               case 3:
                  current.SetParameter(names[0], value, _override: true);
                  current.SetParameter(names[1], key, _override: true);
                  current.SetParameter(names[2], index, _override: true);
                  break;
            }
         }
         else
         {
            switch (names.Length)
            {
               ...
               case 2:
                  current.SetParameter(names[0], value, _override: true);
                  current.SetParameter(names[1], index, _override: true);
                  break;
            }
         }
      }
