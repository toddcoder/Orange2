...
namespace Orange.Library.Values
{
	public class Graph : Value, IMessageHandler
	{
	   ...
	   protected override void registerMessages(MessageManager manager)
		{
			...
			manager.RegisterProperty(this, "value", v => ((Graph)v).GetValue(),
            v => ((Graph)v).SetValue());
	   ...
	   public Value SetValue()
		...
		{
			handled = true;
			if (MessageManager.DefaultRespondsTo(messageName) &&
            !MessageManager.MessagingState.RespondsToRegisteredMessage(value, messageName))
				return MessageManager.DefaultSendMessage(value, messageName, arguments, out handled);
			...
			var closure = childGraph.Value as Lambda;
			if (closure != null)
				return dispatchClosure(closure, arguments);
			var send = childGraph.Children.Count > 0 ? (Value)new Graph(childGraph) :
            new GraphVariable(messageName, childGraph);
			return send;
		}
	   ...
	   Value dispatchClosure(Lambda lambda, Arguments arguments)
		{
			...
			RegionManager.Regions.SetLocal("self", this);
	   ...
	   static void renderXML(ValueGraph graph, Element element)
		...
		public Value If()
		{
			using (var assistant = new ParameterAssistant(Arguments))
			{
				...
				if (graph.Children.Count == 0)
				{
					...
					Value value = block.IsTrue ? graph.Value : "";
		...
		void addToList(ParameterAssistant assistant, Block block, ValueGraph graph, List<Graph> list)
		{
	   ...
	   public Value NewMessage()
		{
			...
			var newGraph = value.As<Graph>();
			if (newGraph.IsSome)
			{
				var innerGraph = newGraph.Value.graph;
		...
		public Value Map()
		{
			using (var assistant = new ParameterAssistant(Arguments))
			{
				...
				var index = 0;
				foreach (KeyValuePair<string, ValueGraph> item in graph.Children)
				{
