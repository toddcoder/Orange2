...
namespace Orange.Library.Values
{
	public class StringNavigator : Value
	{
		...
		{
			return getText().CompareTo(value.Text);
		}
		...
		public override ValueType Type
		{
			get
			{
				return ValueType.StringNavigator;
			}
		}
		...
		{
			return new StringNavigator(text.ToString())
			{
				position = position,
				length = length
			};
		...
		{
			bool found = pattern.IsMatch(textAtPosition());
			if (found)
			{
				int newPosition = pattern.Index;
		...
		{
			int newPosition = textAtPosition().IndexOf(pattern, StringComparison.Ordinal);
		...
		void afterPattern(Pattern pattern)
		{
			...
			if (found)
			{
				int newPosition = pattern.Index + pattern.Length;
				setRelativePosition(newPosition);
			}
			setStatus(found);
		}
		...
		public Value Any()
		{
			...
			char[] needle = Runtime.Expand(value.Text).ToCharArray();
			int textLength = text.Length;
		...
		public override string ToString()
		{
			...
			return length == 0 ? string.Format("{0}|{1}:{2}", text[0, position], text.Substring(position), ch) :
				string.Format("{0}[{1}]{2}:{3}", text[0, position], text[position, length], text.Substring(position + length), ch);
		}
	}
}