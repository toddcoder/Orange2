...
namespace Orange.Library.Values
{
	...
	{
		public static Rational Cast(Value value) => value.As<Rational>().Map(r => r, () => new Rational(value));
	   ...
	   int numerator;
		...
		{
			Rational rational;
			if (value.As<Rational>().Assign(out rational))
	   ...
	   public override int Compare(Value value)
		{
			Rational rational;
			return (value.As<Rational>().Assign(out rational)) ? (numerator * rational.denominator)
            .CompareTo(rational.numerator * denominator) : AlternateValue("cmp").Compare(value);
		}
