	   ...
	   public Block Children
		...
		public virtual Element Element
		{
			get
			{
				...
				{
					Name = name.IsVariable ? ((Variable)name).Name : name.Text
				};
				if (attributes != null)
				{
			   ...
			   {
			      var xml = childrenValue.As<XML>();
			      if (xml.IsSome)
			         children.Add(xml.Value);
			      var array = childrenValue.As<Array>();
			      if (array.IsSome)
