...
namespace Orange.Library.Values
{
	public class Complex : Value
	{
		...
		{
			value = new CSComplex(number.Number, 0);
		}

		public Complex(CSComplex complex)
		{
			value = complex;
		}
		...
		{
			return this.value.Real.CompareTo(value.Number) * this.value.Imaginary.CompareTo(value.Number);
		}

	   public override string Text { get { return ToString(); } set { } }

	   public override double Number { get { return value.Real; } set { } }
	   ...
	   static Value operate(Value other, Func<Complex, Complex> complexFunc, Func<double, Complex> otherFunc)
		{
	      return other.As<Complex>().Map(complexFunc, () => otherFunc(other.Number));
		}
	   ...
	   {
	      return value.Imaginary < 0 ? $"{value.Real}{value.Imaginary}i" : $"{value.Real}+{value.Imaginary}i";
	   }
