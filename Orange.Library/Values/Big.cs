...
namespace Orange.Library.Values
{
	public class Big : Value
	{
		...
		public Big(Value value)
		{
			this.value = new BigInteger(value.Number);
		}

		public Big(string representation)
		{
			BigInteger.TryParse(representation, out value);
			RejectNull(value, "Big", $"Couldn't convert {representation}b");
		}
		...
		public override int Compare(Value other) => other.As<Big>().Map(compareTo, (() => compareTo(other)));
	   ...
	   static Value operate(Value other, Func<Big, Big> bigFunc, Func<double, Big> otherFunc)
		{
	      return other.As<Big>().Map(bigFunc, () => otherFunc(other.Number));
		}
	   ...
	   public Value DivRem() => Arguments[0].As<Big>().Map(big => DivRem(value, big.value),
         () => DivRem(value, (BigInteger)Arguments[0].Number));
