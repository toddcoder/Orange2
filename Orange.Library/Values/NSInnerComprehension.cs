...
namespace Orange.Library.Values
{
   public class NSInnerComprehension : Value, INSGenerator
   {
      ...
      public IMaybe<NSInnerComprehension> NextComprehension
      {
         get;
         set;
      } = new None<NSInnerComprehension>();
      ...
      public INSGeneratorSource GeneratorSource => generatorSource.Evaluate().As<INSGeneratorSource>()
         .Required($"{generatorSource} not a generator source");

      public void Visit(Value value)
      {
      }

      bool ifTrue() => ifBlock.Map(b => b.IsTrue, () => true);

      public void SetValue(Value value) => parameters.SetValues(value, index++);

      public override string ToString()
      {
         return $"{parameters} in {generatorSource}{ifBlock.Map(b => $" if {b} ", () => "")}" +
            $"{NextComprehension.Map(c => $", {c}", () => "")}";
      }
   }
}