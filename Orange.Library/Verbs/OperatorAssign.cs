...
namespace Orange.Library.Verbs
{
   public abstract class OperatorAssign : Verb
   {
      ...
      {
         return (variable.Value.IsArray || value.IsArray) && UseArrayVersion ? evaluateArray(variable, value) : evaluate(variable, value);
      }
         ...
         if (xValue.IsArray)
         {
            ...
            if (value.IsArray)
            {
               ...
               for (var i = 0; i < minLength; i++)
               {
                  ...
                  anonymousVariable = new Variable(variableName) { Value = xItem };
