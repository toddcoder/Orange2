	...
	public class VerbList : IEnumerable<Verb>
	{
	   ...
	   public void Add(Verb verb)
		...
		public void ReplacePlaceholderInvokeWithBody(VerbList verbList)
		{
			...
			{
				InvokePlaceholder invokePlaceholder;
				if (!replaced && verb.As<InvokePlaceholder>().Assign(out invokePlaceholder))
				{
					result.Add(verbList);
					replaced = true;
				}
			...
			for (var i = 0; i < verbs.Count; i++)
			{
				...
				if (verb.As<Push>().Assign(out push))
				{
				   var parameterName = push.Variable();
					...
					Block block;
					if (push.Value.As<Block>().Assign(out block))
					{
				...
				if (verb.As<FunctionInvoke>().Assign(out functionInvoke) && parameters.GetParameters()
				...
				if (verb.As<Dereference>().Assign(out dereference) && i > 0)
				{
					var lastIndex = i - 1;
		...
		public void ReplaceNameWithValue(string name, Value value)
		{
			...
			{
				Push push;
				if (verbs[i].As<Push>().Assign(out push))
				{
	   ...
	   public void ReplaceListParameters(Parameters parameters)
	   {
		...
		public bool ReplaceInvocationWithPlaceholder(string functionName, ref Arguments arguments)
		{
			...
			{
				FunctionInvoke functionInvoke;
				if (verb.As<FunctionInvoke>().Assign(out functionInvoke))
				{
		...
		public VerbList[] Partition(string functionName)
		{
			...
			{
				FunctionInvoke functionInvoke;
				if (verb.As<FunctionInvoke>().Assign(out functionInvoke) && functionInvoke.FunctionName == functionName)
				{
	   ...
	   public override string ToString() => verbs.Listify(" ");
