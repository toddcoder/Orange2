...
/*		static void extractText(Value value, out string text, out PadType padType, out bool useDefault)
		{
			var options = value.Options;
			text = value.Text;
			padType = getPadType(options);
			useDefault = options[Value.OptionType.None];
		}*/

	   static Tuple<string, PadType, bool> extractText(Value value)
	   {
	      var options = value.Options;
	      return tuple(value.Text, getPadType(options), options[Value.OptionType.None]);
	   }
	   ...
	   List<MetaData> metaData;
		...
		public Table(Array array, bool lines = false)
		{
			...
			foreach (var item in array)
			{
				...
				PadType padType;
				bool useDefault;
			   extractText(item.Value).Assign(out text, out padType, out useDefault);
		...
		public void AddRow(Array array)
		{
			...
			for (var i = 0; i < length; i++)
			{
				...
				PadType padType;
				bool useDefault;
			   extractText(array[i]).Assign(out text, out padType, out useDefault);
				if (useDefault)
					row.AddColumn(text);
				else
