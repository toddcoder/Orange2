	...
	public class PatternParser : Parser, IElementParser
	{
		...
		public override Verb CreateVerb(string[] tokens)
		{
			...
			while (scanning && index < sourceLength)
			{
				...
				{
				   var endPatternParser = parser.As<EndPatternParser>();
					if (endPatternParser.IsSome)
					{
						replacement = endPatternParser.Value.Replacement;
						index = endPatternParser.Value.Result.Position;
						found = true;
						break;
					}
				   ...
				   var elementParser = parser.As<IElementParser>();
					if (elementParser.IsNone)
						continue;

					var element = elementParser.Value.Element;
