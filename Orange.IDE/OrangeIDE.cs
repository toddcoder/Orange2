...
/*				settings.CurrentFolder = FolderName.Current;
		...
		void menuToolsInsertInterpolationQuotes_Click(object sender, EventArgs e)
		{
			insertText(@"$out = %%", -1, 0);
		}

		void menuToolsInsertPatternDelimiters_Click(object sender, EventArgs e)
		{
			//insertDelimiterText("[  ]", -2, 0);
			insertDelimiterText("‹  ›", -2, 0);
		}
...
/*			if (e.KeyCode == Keys.Enter && richCode.SelectionLength == 0 && richCode.Text.Sub(richCode.SelectionStart - 2, 3) == "{\n}")
		...
		{
			return richCode.SelectionLength == 0 ? getSelectedLine() : richCode.SelectedText;
		}
